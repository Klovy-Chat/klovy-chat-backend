...
import { broadcastUserStatus, broadcastProfileUpdate } from "./socket/utils/broadcast.js";
...
const setupSocket = (server) => {
    ...
    io.on("connection", (socket) => {
        try {
            ...
            socket.on("add-channel-notify", (channel) => addChannelNotify(channel, userSocketMap, io));
            socket.on("sendMessage", (message) => sendMessage(message, userSocketMap, io));
            socket.on("send-channel-message", (message) => sendChannelMessage(message, userSocketMap, io));
            socket.on("typing", (data) => handleTyping(data, typingUsers, userSocketMap, io));
            socket.on("message-reaction", (data) => handleReaction(data, userSocketMap, io));
            socket.on("mark-message-read", (data) => markMessageAsRead(data, userSocketMap, io));

            socket.on("set-online", async (data) => {
                if (data.userId) {
                    await setUserOnline(data.userId);
                    broadcastUserStatus(data.userId, { isOnline: true, lastSeen: null }, userSocketMap, io);
                }
            });
            ...
            socket.on("set-offline", async (data) => {
                if (data.userId) {
                    ...
                    broadcastUserStatus(data.userId, { isOnline: false, lastSeen: Date.now() }, userSocketMap, io);
                }
            });

            socket.on("editMessage", (data) => handleEditMessage(data, userSocketMap, io));

            socket.on("deleteMessage", (data) => handleDeleteMessage(data, userSocketMap, io));

            socket.on("profile-image-updated", (data) => {
                broadcastProfileUpdate(data.userId, "contact-avatar-updated", { image: data.image }, userSocketMap, io);
            });
    ...
    io.notifyChannelDeleted = (channel) => notifyChannelDeleted(channel, userSocketMap, io);
